...
class ModernMCQApp:
    def __init__(self, root, questions, show_options=True):
        ...
        # Load and resize rocket image
        self.rocket_img_tk = None
        try:
            pil_img = Image.open("rocket.png").rotate(90, expand=True)
            h = 200
        ...
        # If options are off, center timer + question
        if not self.show_options:
            self.center_container = tk.Frame(self.main_frame, bg='#181c2b')
            self.center_container.pack(expand=True)
            self.setup_timer_bar(self.center_container)
            self.setup_question_area(self.center_container, center=True)
        else:
    ...
    def draw_starfield(self):
        ...
        import random
        width = self.bg_canvas.winfo_width() or 900
        height = self.bg_canvas.winfo_height() or 650
        self.stars = []
        for _ in range(120):
    ...
    def setup_timer_bar(self, parent=None):
        parent = parent or self.main_frame
        self.timer_frame = tk.Frame(parent, bg='#181c2b')
    ...
    def setup_question_area(self, parent=None, center=False):
        parent = parent or self.main_frame
        question_frame = tk.Frame(parent, bg='#23264a', relief='flat', bd=1)
        question_frame.pack(pady=(0, 20), padx=40, fill='x')
    ...
    def setup_navigation(self):
        ...
        self.prev_btn = tk.Button(
            nav_frame, text="← Previous", font=('Orbitron', 13, 'bold'),
            fg='#fff', bg='#3a3f5c', activebackground='#5f6fff',
            activeforeground='#fff', relief='flat', bd=0,
            padx=24, pady=12, command=self.prev_question,
            cursor='hand2', highlightthickness=0
        )
        self.prev_btn.pack(side='left', padx=(0, 10))
        self.next_btn = tk.Button(
            nav_frame, text="Next →", font=('Orbitron', 13, 'bold'),
            fg='#fff', bg='#3a3f5c', activebackground='#5f6fff',
            activeforeground='#fff', relief='flat', bd=0,
            padx=24, pady=12, command=self.next_question,
            cursor='hand2', highlightthickness=0
        )
        self.next_btn.pack(side='right', padx=(10, 0))
        self.show_answer_btn = tk.Button(
            nav_frame, text="Reveal Answer", font=('Orbitron', 13, 'bold'),
            fg='#fff', bg='#27ae60', activebackground='#2ecc71',
            activeforeground='#fff', relief='flat', bd=0,
            padx=24, pady=12, command=self.show_answer,
            cursor='hand2', highlightthickness=0
        )
    ...
    def draw_timer_bar(self):
        ...
        self.timer_text = self.timer_canvas.create_text(width//2, bar_height//2, text=f"{self.timer_remaining:02d} s", fill='#fff', font=('Inter', 14, 'bold'), anchor='center')
    ...
    def show_answer(self):
        ...
        for i, option in enumerate(q['options']):
            if option.strip() == correct_answer.strip():
                correct_index = i
                break
        if correct_index is not None and self.show_options:
            self.option_labels[correct_index].configure(bg='#27ae60', fg='white')
            messagebox.showinfo("Correct Answer", f"The correct answer is: {chr(65 + correct_index)}. {q['options'][correct_index]}")
        else:
            messagebox.showwarning("Answer Not Found", f"Could not find the answer '{correct_answer}' in the options.")
...
def select_file_and_run():
    filename = filedialog.askopenfilename(title="Select MCQ File", filetypes=[('Text Files', '*.txt'), ('All Files', '*.*')])
    if filename:
        try:
            questions = parse_mcq_file(filename)
            if questions:
                show_options = messagebox.askyesno("Display Options", "Do you want to display the multiple choice options?")
                root = tk.Tk()
